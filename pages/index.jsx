...
export default function Home() {
  ...
  const numericTime = Number.isFinite(Number(time)) ? Math.max(1, Math.min(180, parseInt(time || 0, 10))) : 10;
  const canStart = Boolean(userId.trim()) && Boolean(selectedPaper) && numericTime >= 1;

  const handleStart = () => {
    if (!canStart) return;
    router.push(`/quiz?paper=${selectedPaper}&time=${numericTime}&userId=${encodeURIComponent(userId)}`);
  };
    ...
    <div className="min-h-screen bg-gradient-to-br from-sky-50 via-white to-indigo-50 dark:from-gray-950 dark:via-gray-900 dark:to-gray-950">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl sm:text-2xl font-extrabold text-gray-900 leading-tight dark:text-gray-100">
                Nursing Officer Practice
              </h1>
              <p className="text-xs sm:text-sm text-gray-500 dark:text-gray-400">
                KGMU · SGPGI — question index & quizzes
              </p>
            </div>
          </div>
      ...
      <main className="mx-auto max-w-6xl px-4 py-8">
        ...
        <div className="rounded-3xl bg-white shadow-sm ring-1 ring-gray-200/60 p-4 sm:p-6 dark:bg-gray-900 dark:ring-gray-800">
          ...
          {/* Shared User ID */}
          <div className="mt-4">
            <label className="block text-sm font-semibold text-gray-700 mb-1 dark:text-gray-200">User ID</label>
              ...
              <div className="mt-5 space-y-4">
                ...
                <div>
                  <div className="flex items-center justify-between">
                    <label className="block text-sm font-semibold text-gray-700 dark:text-gray-200">Timer (mins)</label>
